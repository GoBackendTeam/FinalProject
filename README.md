...
> macOS 注意:本服務預設「直接跑在主機」(`make run`),judge 容器的 bind mount
> 路徑與服務看到的一致,最單純。若要把 app 也容器化,見下方「全容器啟動」。
...
```bash
make keys
docker compose up -d --build   # 需在專案根目錄執行(${PWD} 會掛載 workspace/problems)
```

compose 內的 app 透過 `docker.sock` 啟動 judge 容器,並以 `PROBLEMS_ROOT_HOST` /
`JUDGE_WORKSPACE_HOST` 把 bind mount 指回主機路徑。
...
OP=$(curl -s -H "Authorization: Bearer $TOKEN" -F problem_id=113final003 -F file=@/tmp/sol.zip \
...
- `PROBLEMS_ROOT_HOST` — 同上,題庫目錄的主機路徑(供 judge bind mount)。
